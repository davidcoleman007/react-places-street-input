...
import ReactPlacesStreetInput, { geocodeByAddress } from '../../src'
...
describe('<Demo />', () => {
  ...
  it('renders a ReactPlacesStreetInput component', () => {
    expect(wrapper.find(ReactPlacesStreetInput)).to.exist
  })

  it('sets the value prop of the input element to state.address', () => {
    const wrapper = mount(<Demo />)
    wrapper.setState({ address: '123 Fake St' })
    expect(wrapper.find(ReactPlacesStreetInput).props().value).to.equal('123 Fake St')
  })
