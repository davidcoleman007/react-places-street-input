...
import ReactPlacesStreetInput, { geocodeByAddress } from '../index.js'
...
/*** Enzyme Rocks ***/
describe('<ReactPlacesStreetInput />', () => {
  let wrapper;
  beforeEach(() => {
    wrapper = shallow(<ReactPlacesStreetInput inputProps={testInputProps} />)
  })
...
describe('ReactPlacesStreetInput callbacks', () => {
  it('executes onError callback passed in as prop when status is not OK', () => {
    const spy = sinon.spy()
    const wrapper = mount(<ReactPlacesStreetInput onError={spy} inputProps={testInputProps} />)
  ...
  it('executes default onError function when there is no custom prop and status is not OK', () => {
    ...
    const wrapper = mount(<ReactPlacesStreetInput inputProps={testInputProps} />)
    wrapper.instance().autocompleteCallback([], 'ZERO_RESULTS')
    expect(console.error.calledOnce).to.equal(true)
  })
});

describe('ReactPlacesStreetInput props', () => {
  it('allows user to set the value of the input through prop', () => {
    ...
    const wrapper = mount(<ReactPlacesStreetInput inputProps={inputProps} />)
...
describe('autocomplete dropdown', () => {
  ...
  beforeEach(() => {
    wrapper = shallow(<ReactPlacesStreetInput inputProps={testInputProps} autocompleteItem={autocompleteItem} />)
  })

  it('initially does not have an autocomplete dropdown', () => {
    expect(wrapper.find('#ReactPlacesStreetInput__autocomplete-container')).to.have.length(0)
  })
  ...
  it('renders autocomplete dropdown once it receives data from google maps', () => {
    ...
    expect(wrapper.find('#ReactPlacesStreetInput__autocomplete-container')).to.have.length(1)
    ...
    describe('the Google logo', () => {
      it('is displayed by default', () => {
        wrapper = shallow(<ReactPlacesStreetInput inputProps={testInputProps} autocompleteItem={autocompleteItem} />)
        wrapper.setState({ autocompleteItems: data })
        expect(wrapper.find('#ReactPlacesStreetInput__google-logo')).to.have.length(1)
      })

      it('is not displayed when toggled off', () => {
        wrapper = shallow(<ReactPlacesStreetInput inputProps={testInputProps} autocompleteItem={autocompleteItem} googleLogo={false} />)
        wrapper.setState({ autocompleteItems: data })
        expect(wrapper.find('#ReactPlacesStreetInput__google-logo')).to.have.length(0)
      })
    })
  })
  ...
  it('clears the autocomplete items when PlacesServiceStatus is not OK and clearItemsOnError prop is true', () => {
    ...
    const wrapper = shallow(<ReactPlacesStreetInput inputProps={testInputProps} clearItemsOnError={true}/>)
...
describe('custom classNames, placeholder', () => {
  ...
  beforeEach(() => {
    wrapper = shallow(<ReactPlacesStreetInput inputProps={inputProps} classNames={classNames} />)
  })
  ...
  it('lets you set a custom className for autocomplete container', () => {
    ...
    expect(wrapper.find('#ReactPlacesStreetInput__autocomplete-container')).to.have.className('my-autocomplete-container')
  })
...
describe('customizable autocompleteItem', () => {
  it('lets you provide a custom autocomplete item', () => {
    ...
    const wrapper = shallow(<ReactPlacesStreetInput inputProps={testInputProps} autocompleteItem={autocompleteItem}/>)
    wrapper.setState({ autocompleteItems: [{ suggestion: 'San Francisco, CA', placeId: 1, active: false, index: 0 }] })
...
describe('custom inline styles', () => {
  ...
  beforeEach(() => {
    ...
    wrapper = shallow(<ReactPlacesStreetInput inputProps={testInputProps} styles={styles} />)
  })

  it('lets you set custom styles for the root element', () => {
    expect(wrapper.find('#ReactPlacesStreetInput__root').props().style.position).to.equal('absolute')
  })
  ...
  it('lets you set custom styles for the autocomplete container element', () => {
    wrapper.setState({ autocompleteItems: [{ suggestion: 'San Francisco, CA', placeId: 1, active: false, index: 0 }] })
    expect(wrapper.find('#ReactPlacesStreetInput__autocomplete-container').props().style.backgroundColor).to.equal('green')
  })

  it('lets you set custom styles for autocomplete items', () => {
    wrapper.setState({ autocompleteItems: [{ suggestion: 'San Francisco, CA', placeId: 1, active: false, index: 0 }] })
    const item = wrapper.find("#ReactPlacesStreetInput__autocomplete-container").childAt(0)
 ...
 describe('AutocompletionRequest options', () => {
   ...
   it('calls getPlacePredictions with the correct options', (done) => {
     ...
     const wrapper = mount(<ReactPlacesStreetInput inputProps={inputProps} options={options} />)
